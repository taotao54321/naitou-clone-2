...
fn main() -> anyhow::Result<()> {
    ...
    // 現局面の COM 側の駒数が残り深さより大きければ明らかに解なし。
    // 両者が等しければ、駒取りの指し手のみを生成。
    // それ以外の場合、通常の指し手を生成。
    let mvs = {
        let pos = engine.position();
        if opt.depth.get() < pos.com_nonking_count() {
            return Ok(());
        } else if opt.depth.get() == pos.com_nonking_count() {
        ...
        if let Ok(resp) = self.engine.do_step(mv_first) {
            ...
            match resp {
                ...
                EngineResponse::HumWin(_) => {
                    ...
                    if self.engine.position().com_nonking_count() == 0 {
                        self.print_solution();
                    }
                }
                // HUM 負けなら何もしない。
